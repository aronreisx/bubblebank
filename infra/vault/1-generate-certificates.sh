...
# 1.1. Make directories for certs
mkdir -p tls
cd tls

# 1.2. Generate a root CA private key and self-signed CA cert
...
# 1.3. Generate Vault server private key & CSR (common name = vault.local)
...
# 1.4. Create a config file for SANs (so “localhost” and “vault.local” both work)
...
# 1.5. Sign the CSR with the CA, adding SANs
